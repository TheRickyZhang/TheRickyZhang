...
def main():
    ...
    # tally lines and per-repo contributions
    lang_tally = Counter()
    ...
    for lang, cnt in top10:
        ...
        print(f"| {lang:<11} | {c:>8,} | {pct:>9.2f}% | [{name}]({url}) |")
    ...
    for lang, cnt in top10:
        pct = cnt / total * 100
        repo, _ = lang_repo[lang].most_common(1)[0]
        name = repo.split('/', 1)[1]
        url = f"https://github.com/{repo}"
        output.append(f"| {lang:<11} | {cnt:>6,} | {pct:>9.2f}% | [{name}]({url}) |")
    new_section = "\n".join(output)
